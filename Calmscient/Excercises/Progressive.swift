...
class Progressive: ViewController {
    ...
    @IBOutlet weak var completeButton: CapsuleButton1!
        ...
        private var isRetrying = false  // FIX: guard against rapid duplicate retries
        ...
        private func retryPlayer() {
            // FIX: prevent multiple rapid retries from quick taps
        ...
        override func observeValue(forKeyPath keyPath: String?,
            ...
            if keyPath == "timeControlStatus" {
                ...
                case .readyToPlay:
                    isPlayerReady = true
                    needsPlayerRetry = false
                    isRetrying = false  // FIX: reset retry flag on success
                    self.view.hideToastActivity()
                    playAudioImg.isHidden = false
                    
                case .failed:
                    isPlayerReady = false
                    isRetrying = false  // FIX: reset retry flag on failure so user can try again
                    self.view.hideToastActivity()
                    playAudioImg.isHidden = false
                    invalidateProgressTimer()
        ...
        @objc func playPauseTapped(tapGestureRecognizer: UITapGestureRecognizer) {
            ...
            guard item.status == .readyToPlay else {
                if needsPlayerRetry {
                    // FIX: debounce 0.3s to let NetworkMonitor settle before reading isConnected
