//
//  HoldYourLiquorViewModel.swift
//  Calmscient
//
//  State, API, and navigation for the hold-your-liquor screen (parity with `HoldYourLiquorVC`).
//
//  Vivek
//  25 May 2026
//
...
@available(iOS 16.0, *)
@MainActor
final class HoldYourLiquorViewModel: ObservableObject {

    weak var hostViewController: UIViewController?

    @Published private(set) var content = HoldYourLiquorContentPresentation(
        headerTitle: "",
        bodyParagraphs: []
    )
    ...
    func reloadLocalizedStrings() {
        ...
        content = HoldYourLiquorPresentation.buildLocalizedContent()
    }
    ...
    private func handleCompleteResponse(_ response: AnyObject) {
        ...
        guard response is [String: Any] else {
            print("HoldYourLiquorViewModel: unsupported complete response — \(type(of: response))")
            return
        }
...
enum HoldYourLiquorNavigation {
    ...
    static func push(
        ...
        let holdYourLiquorHost = HoldYourLiquorHostingController()
        holdYourLiquorHost.viewModel.configure(sectionId: sectionId, navigationTitle: navigationTitle)
        nav.pushViewController(holdYourLiquorHost, animated: animated)
    }
}
