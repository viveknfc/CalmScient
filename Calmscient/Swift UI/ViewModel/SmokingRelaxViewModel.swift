//
//  SmokingRelaxViewModel.swift
//  Calmscient
//
//  State, API, and navigation for the smoking relax screen (parity with `SmokingRelaxVC`).
//
//  Vivek
//  26 May 2026
//
...
@available(iOS 16.0, *)
@MainActor
final class SmokingRelaxViewModel: ObservableObject {
    ...
    func reloadLocalizedStrings() {
        ...
        content = SmokingRelaxPresentation.buildLocalizedContent()
    }
    ...
    private func handleCompleteResponse(_ response: AnyObject) {
        ...
        guard response is [String: Any] else {
            print("SmokingRelaxViewModel: unsupported complete response — \(type(of: response))")
            return
        }
...
enum SmokingRelaxNavigation {
    ...
    static func push(
        ...
        let smokingRelaxHost = SmokingRelaxHostingController()
        smokingRelaxHost.viewModel.configure(sectionId: sectionId, navigationTitle: navigationTitle)
        nav.pushViewController(smokingRelaxHost, animated: animated)
    }
}
