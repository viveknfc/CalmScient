//
//  SmokingAffectMentalHealthViewModel.swift
//  Calmscient
//
//  State, API, and navigation for the smoking and mental health screen (parity with `SmokingAffectMentalHealthVC`).
//
//  Vivek
//  26 May 2026
//
...
@available(iOS 16.0, *)
@MainActor
final class SmokingAffectMentalHealthViewModel: ObservableObject {
    ...
    private func handleCompleteResponse(_ response: AnyObject) {
        ...
        guard response is [String: Any] else {
            print("SmokingAffectMentalHealthViewModel: unsupported complete response — \(type(of: response))")
            return
        }
...
@available(iOS 16.0, *)
enum SmokingAffectMentalHealthNavigation {
    ...
    static func push(
        ...
        let mentalHealthHost = SmokingAffectMentalHealthHostingController()
        mentalHealthHost.viewModel.configure(sectionId: sectionId, navigationTitle: navigationTitle)
        nav.pushViewController(mentalHealthHost, animated: animated)
    }
}
