//
//  MyDrinkingHabitHostingController.swift
//  Calmscient
//
//  UIKit shell for `MyDrinkingHabitView` (navigation, `NavigationBack`, tab bar).
//
//  Vivek
//  25 May 2026
//
...
@available(iOS 16.0, *)
final class MyDrinkingHabitHostingController: UIViewController {

    let viewModel = MyDrinkingHabitViewModel()
    private var hostingController: UIHostingController<MyDrinkingHabitView>!
    private var navigationBackHostingController: UIHostingController<MedicationNavigationBackButton>?

    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = UIColor(named: "AppViewContentColor") ?? .systemBackground

        viewModel.hostViewController = self

        hostingController = UIHostingController(rootView: MyDrinkingHabitView(viewModel: viewModel))
