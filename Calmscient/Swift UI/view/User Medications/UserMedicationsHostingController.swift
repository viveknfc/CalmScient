//
//  UserMedicationsHostingController.swift
//  Calmscient
//
//  UIKit shell for `UserMedicationsView` (navigation, bottom-sheet date jump).
//
//  Vivek
//  14 May 2026
//
...
@available(iOS 16.0, *)
final class UserMedicationsHostingController: UIViewController {

    private let viewModel = UserMedicationsViewModel()
    ...
    override func viewDidLoad() {
        ...
        hostingController = UIHostingController(rootView: UserMedicationsView(viewModel: viewModel))
    ...
    private func presentMonthDatePickerFromBottom() {
        ...
        monthDatePickerPresenter.present(from: self, configuration: configuration) { [weak self] date, isTimePicker in
            ...
            self?.viewModel.selectMedicationDateFromMonthPicker(date)
        }
    }
}
