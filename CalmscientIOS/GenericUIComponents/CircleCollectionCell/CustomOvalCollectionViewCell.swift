//
//  CustomOvalCollectionViewCell.swift
//  CalmscientIOS
//
...
class CustomOvalCollectionViewCell: UICollectionViewCell {
    ...
    public func addCircularShape() {
        ...
        let estimatedWidth = self.bounds.width * (0.8)
        let estimatedHeight = self.bounds.height
        let originX = (self.bounds.width - estimatedWidth) / 2
        circleShapeLayer.path = UIBezierPath(ovalIn: CGRect(x: originX, y: 0, width: estimatedWidth, height: estimatedHeight)).cgPath
